...
// Case-sensitive v1
const COMPACT_TS_RE = /^(?<authority>(?:[hH][tT][tT][pP][sS]?:\/\/)?[[\]0-9a-z_.:-]+(?::[0-9]+)?)\/(?<interval>[0-9]+)#v1(?=[:,])(?::(?<mth>[a-zA-Z0-9_-]+))?(?:,(?<timestamp>[^,]+),(?<proof>[a-zA-Z0-9_-]+))?$/
...
export class TimestampService {
    ...
    private readonly _listeners: { [key: number]: (item: TickItemDisplay) => void }
    ...
    constructor(authority: string) {
        ...
        this.baseUrl = (!this.authority.includes('://') ? 'https://' : '') + this.authority + '/api/'
        this.cacheIth = {}
        this.cacheMtree = {}
        this.knownHead = null
        this.ws = null
        ...
        console.log(`Created new TimestampService for ${this.authority} at ${this.baseUrl}`)
    }
    ...
    get averageTickDurationMillis() {
        const diffs = this.tickItems
            .slice(1)
            .map((v, i) => this.tickItems[i].received.getTime() - v.received.getTime())
            .filter((v) => v >= 1000)
        if (diffs.length > 0) {
    ...
    openLiveConnection() {
        ...
        this.ws = new WebSocket(this.baseUrl.replace(/^http/i, 'ws') + 'v1/mth/live')
    ...
    updateState(authority: string, type: 'mh' | 'mth', data: UpdateInput) {
        ...
        if (type === 'mh') {
            const mh = data.mh!
            if (!this.knownHead?.interval || this.knownHead.interval < mh.interval.index) {
                this.knownHead = {
                    interval: mh.interval.index, mth: Buffer.from(mh.mth, 'base64'),
                }
            }
            this.cacheIth[mh.interval.index] = Buffer.from(mh.interval.ith, 'base64')
            this.cacheMtree[`0-${mh.interval.index + 1}`] = Buffer.from(mh.mth, 'base64')
        }
    }

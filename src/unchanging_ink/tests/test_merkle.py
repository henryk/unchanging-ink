...
from unchanging_ink.crypto import MerkleNode, MerkleTree, consistency_proof_nodes, verify_consistency_proof


def test_tree_none():
    a, idx = MerkleNode.from_sequence_with_index([])
    assert a.value == bytes.fromhex(
...
def test_tree_four():
    ...
    assert a.height == 3
    assert idx[(0, 1)].value == bytes.fromhex(
        "b8244d028981d693af7b456af8efa4cad63d282e19ff14942c246e50d9351d22704a802a71c3580b6370de4ceb293c324a8423342557d4e5c38438f0e36910ee"
    )
...
def test_proof_verify(target, length):
    head_node, idx = MerkleNode.from_sequence_with_index(
